...
main_folder <- r"(E:\new versions\v7_17clusters_MD4)" # direct to all deployments
...
write.csv(day.results,"E:\\Bp.csv",row.names=FALSE)
...
fig.results <- day.results %>% 
  ...
  group_by(deployment) %>%
  mutate(total_days = sum(days_detect + days_nodetect + days_seidetect),
         percent = (days_detect / total_days) * 100) %>%
  ungroup() %>%
  
  select(-total_days) %>% 
  mutate(year = str_extract(deployment, "\\d{4}")) %>% 
  
  pivot_longer(cols = c(days_detect, days_nodetect,days_seidetect),
               names_to = "detection",
               values_to = "days") %>% 
...
for (i in years) {
  ...
  p <-ggplot()+
    ...
    geom_text(data = percent.label, 
              aes(x = station, y= days+2, label = paste0(round(percent), "%"))) +
    
    labs(title = paste0(i," Deployments: ", sp_label[single_sp]),
         x = "Station",
